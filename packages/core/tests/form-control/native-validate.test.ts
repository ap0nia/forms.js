...
import type { FieldError } from 'packages/core/src/types/errors'
...
describe('FormContol', () => {
  ...
  test('single invalid field array input', async () => {
    ...
    /**
     * Once parsed into the form control's state, the error will be recognized as a field array error.
     * Which means it will be grouped under the `root` property.
     */
    const expectedErrors: Record<string, FieldError> = {
      name: {
        root: {
