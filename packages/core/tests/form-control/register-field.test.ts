...
describe('FormContol', () => {
  describe('registerField', () => {
    ...
    describe('never notifies subscribers', () => {
      test('does not notify any direct subscribers to state', () => {
        const formControl = new FormControl()

        const fn = vi.fn()

        formControl.state.errors.subscribe(fn)
        formControl.state.defaultValues.subscribe(fn)
        formControl.state.dirtyFields.subscribe(fn)
        formControl.state.disabled.subscribe(fn)
        formControl.state.isDirty.subscribe(fn)
        formControl.state.isLoading.subscribe(fn)
        formControl.state.isSubmitted.subscribe(fn)
        formControl.state.isSubmitting.subscribe(fn)
        formControl.state.isSubmitSuccessful.subscribe(fn)
        formControl.state.isValid.subscribe(fn)
        formControl.state.isValidating.subscribe(fn)
        formControl.state.touchedFields.subscribe(fn)
        formControl.state.values.subscribe(fn)
        formControl.state.submitCount.subscribe(fn)
      ...
      test('does not notify any subscribers to derived state', () => {
        const formControl = new FormControl()

        const fn = vi.fn()

        // Track all derived state properties.
        ...
        formControl.derivedState.proxy.disabled
        formControl.derivedState.proxy.isDirty
        formControl.derivedState.proxy.isLoading
        formControl.derivedState.proxy.isSubmitted
        formControl.derivedState.proxy.isSubmitting
        formControl.derivedState.proxy.isSubmitSuccessful
