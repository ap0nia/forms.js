...
import { describe, expect, it, test, vi as jest } from 'vitest'
...
describe('register', () => {
  ...
  describe('register disabled', () => {
    ...
    it('should validate value after toggling enabled/disabled on input', async () => {
      ...
      await waitFor(() => expect(submit).toBeCalledWith({ test: defaultValue }))
  ...
  it('should not register nested input', () => {
      ...
      return (
        <form>
          <Controller
            name="test"
            control={control}
            render={({ field }) => <Checkboxes onChange={field.onChange} value={field.value} />}
          />
          <input type="submit" />
        </form>
      )
    ...
    expect(watchedValue).toEqual([{ test: [true, false, false] }, { test: [false, false, false] }])
