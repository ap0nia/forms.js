...
describe('useFieldArrayUnregister', () => {
  it('should behaviour correctly', () => {
    cy.visit('/use-field-array-unregister')
    ...
    cy.get('#touched').should(($state) =>
      expect(JSON.parse($state.text())).to.be.deep.equal([{ name: true, conditional: true }]),
    )
    ...
    cy.get('#touched').should(($state) =>
      expect(JSON.parse($state.text())).to.be.deep.equal([null, { name: true, conditional: true }]),
    )
    ...
    cy.get('#dirtyFields').should(($state) =>
      expect(JSON.parse($state.text())).to.be.deep.equal({
        data: [{ name: true }, { name: false }, { name: true, conditional: true }, { name: true }],
      }),
    )
