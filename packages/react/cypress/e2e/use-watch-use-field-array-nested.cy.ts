describe('useWatchUseFieldArrayNested', () => {
  it('should watch the correct nested field array', () => {
    cy.visit('/use-watch-use-field-array-nested')
    ...
    cy.get('#result').should(($state) =>
      expect(JSON.parse($state.text())).to.be.deep.equal([
        {
          firstName: 'Bill',
          keyValue: [{ name: 'insert' }, { name: '1a' }, { name: '1c' }, { name: 'append' }],
          lastName: 'Luo',
        },
      ]),
    )
    ...
    cy.get('#result').should(($state) =>
      expect(JSON.parse($state.text())).to.be.deep.equal([
        ...
        {
          firstName: 'Bill',
          keyValue: [{ name: 'insert' }, { name: '1a' }, { name: 'update' }, { name: 'append' }],
          lastName: 'Luo',
        },
      ]),
    )
    ...
    cy.get('#result').should(($state) =>
      expect(JSON.parse($state.text())).to.be.deep.equal([{ firstName: 'prepend', keyValue: [] }]),
    )
